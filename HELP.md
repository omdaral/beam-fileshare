...
> Current version: `v1.6.0` (check the badge on the home page).
...
- Releases page shows `404`? The repo is still **Private** or the `v1.6.0` tag was never pushed — make the repo Public and run `git push origin main v1.6.0`, then the `Release` workflow publishes all assets.
  صفحة Releases تعطي `404`؟ المستودع ما زال **خاصاً** أو تاج `v1.6.0` لم يُدفع — اجعله Public ونفّذ الدفع، وسيبني الـ workflow كل الأصول تلقائياً.
...
- iPhone: no native app — Safari → **Add to Home Screen**. Android native APK: download from [Releases](https://github.com/AhmedFaseh/beam-fileshare/releases/latest/download/Beam-1.6.0-android.apk) (allow "unknown sources" once); build guide in `mobile/SETUP.md` is for developers only.
