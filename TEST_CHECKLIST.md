...
- [ ] `bash get-beam.sh --install --dir /tmp/beam-install-test` installs folder + single-file `~/.local/bin/beam` + menu entry, leaves no archive/temp behind; `--uninstall --dir` removes everything
